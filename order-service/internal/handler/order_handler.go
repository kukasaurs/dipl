...
func (h *OrderHandler) GetMyAssignedOrders(c *gin.Context) {
	userID := c.GetString("userId")
	orders, err := h.service.FilterOrders(c.Request.Context(), map[string]interface{}{
		"cleaner_id": userID,
	})
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
...
func (h *OrderHandler) GetOrderDetails(c *gin.Context) {
	...
	if order.CleanerID == nil || *order.CleanerID != userID {
...
func (h *OrderHandler) RejectAssignedOrder(c *gin.Context) {
	userID := c.GetString("userId")
	id, err := primitive.ObjectIDFromHex(c.Param("id"))
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid order ID"})
	...
	err = h.service.RejectOrder(c.Request.Context(), id, userID)
	if err != nil {
		c.JSON(http.StatusForbidden, gin.H{"error": err.Error()})
		return
	}

	c.JSON(http.StatusOK, gin.H{"message": "Order unassigned"})
}
